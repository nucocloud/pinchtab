...
// BuildPersona(launchUA, cfg.ChromeVersion), which synthesizes a UA from
...
func TestWorkerStealthParityDefersToNativeUAWhenNoCustomUA(t *testing.T) {
	cfg := &config.RuntimeConfig{ChromeVersion: "144.0.7559.133"}
	...
	persona := workerStealthPersona(launchUA, cfg.ChromeVersion)
	if persona.UserAgent != "" {
		t.Fatalf("worker persona must defer to native nav.userAgent when no custom UA is configured; got %q (this is forced onto the worker and creates a page/worker UA mismatch when installed Chrome is not %s)", persona.UserAgent, cfg.ChromeVersion)
	}
...
func TestWorkerStealthParityAppliesCustomUserAgent(t *testing.T) {
	...
	cfg := &config.RuntimeConfig{ChromeVersion: "144.0.7559.133", UserAgent: customUA}
	bundle := stealth.NewBundle(cfg, 1)
